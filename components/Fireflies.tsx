...
export function Fireflies({ count = 36 }: { count?: number }) {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      for (const f of flies) {
        ...
        const lit = p < f.duty ? 0.5 - 0.5 * Math.cos((p / f.duty) * Math.PI * 2) : 0;
    ...
    const step = (dt: number, t: number) => {
      for (const f of flies) {
        f.x += (f.vx + Math.sin(t * f.w1 + f.ph1) * 18 + Math.sin(t * f.w2 + f.ph2) * 9) * dt;
        f.y += (f.vy + Math.cos(t * f.w1 * 0.8 + f.ph2) * 14 + Math.sin(t * f.w2 + f.ph1) * 7) * dt;
